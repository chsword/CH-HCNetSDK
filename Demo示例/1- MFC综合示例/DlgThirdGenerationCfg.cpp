...
void CDlgThirdGenerationCfg::OnButtonSetsmsrelative() 
{
	...
	if (!NET_DVR_SetDVRConfig(m_lServerID, NET_DVR_SET_SMSRELATIVEPARA, -1, &m_struSmsRelativeParam, sizeof(m_struSmsRelativeParam)))
	{
		MessageBox(NET_DVR_GetErrorMsg());
		return;
	}
...
void CDlgThirdGenerationCfg::OnButtonGetsmsrelative() 
{
	...
	if (!NET_DVR_GetDVRConfig(m_lServerID, NET_DVR_GET_SMSRELATIVEPARA, -1, &m_struSmsRelativeParam, sizeof(m_struSmsRelativeParam), &dwReturn))
	{
		char szLan[16] = {0};
