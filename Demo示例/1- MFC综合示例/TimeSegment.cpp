...
void CTimeSegment::UpdateList()
{
    ...
    int iIndex;
#ifdef DEMO_LAN_CN
    const char* pDoorStatus[] = { "无效", "休眠", "常开(自由)", "常闭(禁用)" };
    const char *pVerify[] = { "无效", "休眠", "刷卡+密码", "刷卡", "刷卡或密码", "指纹", "指纹+密码", "指纹或刷卡", "指纹+刷卡", "指纹+刷卡+密码", "人脸或指纹或刷卡或密码", "人脸+指纹", \
        "人脸+密码", "人脸+刷卡", "人脸", "工号+密码", "指纹或密码", "工号+指纹", "工号+指纹+密码", "人脸+指纹+刷卡", "人脸+密码+指纹", "工号+人脸", "人脸或人脸+刷卡" };
#else
    const char* pDoorStatus[] = {"invalid", "sleep", "Normally open", "Normally close"};
    ...
    for (int i=0; i<MAX_TIMESEGMENT_V30; i++)
    {
        ...
        if ( m_struTimeSegment[i].byVerifyMode > 23)
        {
