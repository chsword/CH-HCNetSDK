...
typedef struct tagNET_DVR_AID_ALARM_V100
{
    DWORD              dwSize;              //结构长度
    ...
    BYTE               byRes[20]; // 保留字节   
}NET_DVR_AID_ALARM_V100, *LPNET_DVR_AID_ALARM_V100;
...
BOOL CClientDemoDlg::OnInitDialog()
{
    ...
    int i = sizeof(NET_DVR_AID_ALARM_V41);
    int j = sizeof(NET_VCA_DEV_INFO);
    int k = sizeof(NET_DVR_AID_INFO);
    int n = sizeof(NET_DVR_SCENE_INFO);
    int dw = sizeof(DWORD);
    int n100 = sizeof(NET_DVR_AID_ALARM_V100);
...
void *CClientDemoDlg::DoLoginEx(void *lpParam)
{
	...
	if (g_struDeviceInfo[iDeviceIndex].lLoginID < 0)
	{
		...
		if (g_struDeviceInfo[iDeviceIndex].iDeviceType == DS_B20_MSU_NP || \
            ...
            g_struDeviceInfo[iDeviceIndex].iDeviceType == DS_C20N_VWMS) //B20/B21主控
		{
			int nAllChanNums = 0;
			int nPreChanNums = 0;
...
void CClientDemoDlg::ProcPdcAlarm(WPARAM wParam, LPARAM lParam)
{
    ...
    if (struPDCAlarmInfo.byMode == 0 )
    {
        ...
        sprintf(szInfoBuf, "Abs[%4.4d%2.2d%2.2d%2.2d%2.2d%2.2d] Rel[%4.4d%2.2d%2.2d%2.2d%2.2d%2.2d] DevIP:[%s]Port[%d]Channel[%d] IvmsChannel[%d]IvmsChannelEx[%d] single frame:Channel[%d]LeaveNum[%d]EnterNum[%d]Passing[%d]Smart[%d] BrokenNetHttp[%d] ",
    ...
    {
        sprintf(szInfoBuf, "DevIP:[%s]Port[%d]Channel[%d]:time range:Start Y[%d]M[%d]D[%d]H[%d]M[%d]S[%d]  End Y[%d]M[%d]D[%d]H[%d]M[%d]S[%d]IvmsChannel[%d]IvmsChannelEx[%d]Channel[%d]LeaveNum[%d]EnterNum[%d]Passing[%d]Smart[%d]BrokenNetHttp[%d]Mode[%d]", 
            struPDCAlarmInfo.struDevInfo.struDevIP.sIpV4, struPDCAlarmInfo.struDevInfo.wPort, struPDCAlarmInfo.struDevInfo.byChannel,
...
void CClientDemoDlg::ProcAidAlarmV41(WPARAM wParam,LPARAM lParam)
{
    ...
    sprintf(szInfoBuf, "Aid AlarmV41 xmlLen[%d]IvmsChan[%d]ID[%d]Name[%s]Type[%d]newType[%s] LaneNo[%d] Abs[%4.4d%2.2d%2.2d%2.2d%2.2d%2.2d]Dev IP[%s]Port[%d]Chan[%d] \
        Direction Start[%0.3f,%0.3f]Stop[%0.3f,%0.3f] SceneID[%d] SceneName[%s] Direction[%d] PTZ[%d,%d,%d] MonitoringSiteID[%s] DeviceID[%s] VehicleEnterState[%d]", 
        ...
        struAidAlarm.byMonitoringSiteID, struAidAlarm.byDeviceID, struAidAlarm.struAIDInfo.byVehicleEnterState);
...
void CClientDemoDlg::ProcTfsAlarm(WPARAM wParam,LPARAM lParam)
{
    ...
    sprintf(szInfoBuf, "Tfs Alarm IvmsChan[%d]ID[%d]Name[%s]Type[%d]Abs[%4.4d%2.2d%2.2d%2.2d%2.2d%2.2d]DevIP[%s]Port[%d]Chan[%d]SpecificVehicleType[%d]\
        ...
        PilotSunVisor[%d]CopilotSunVisor[%d]PilotCall[%d]IllegalCode[%s] VehicleEnterState[%d]",
        ...
        struTfsAlarm.byCopilotSunVisor, struTfsAlarm.byPilotCall, struTfsAlarm.byIllegalCode, struTfsAlarm.struAIDInfo.byVehicleEnterState);
...
void CClientDemoDlg::ProcFaceSnapAlarm(WPARAM wParam, LPARAM lParam)
{
    ...
    sprintf(szInfoBuf, "Face Snap alarm Abs[%4.4d%2.2d%2.2d%2.2d%2.2d%2.2d] FaceID[%d] Smart[%d] AgeGroup[%d] Sex[%d] EyeGlass[%d] \
        ...
        struFaceSnapAlarm.struDevInfo.byIvmsChannel, struFaceSnapAlarm.wDevInfoIvmsChannelEx, struFaceSnapAlarm.struFeature.byAge, struFaceSnapAlarm.struFeature.byAgeDeviation);
    AddLog(m_iCurDeviceIndex, ALARM_INFO_T, szInfoBuf);
...
void CClientDemoDlg::ProcFaceSnapMatchAlarm(WPARAM wParam, LPARAM lParam)
{
    ...
    {
        memcpy(temppID, struFaceSnapMatchAlarm.struBlackListInfo.struBlackListInfo.pFDDescriptionBuffer, struFaceSnapMatchAlarm.struBlackListInfo.struBlackListInfo.dwFDDescriptionLen);
    }
    sprintf(szInfoBuf, "Face Snap Match alarm Abs[%4.4d%2.2d%2.2d%2.2d%2.2d%2.2d] FaceID[%d] BlackList ID[%d] StorageIP[%s]-StoragePort[%d] Sex[%d] Glasses[%d] Age[%d] MatchPicNum[%d] struRegion[%0.1f, %0.1f, %0.1f, %0.1f] PicTransType[%d]  FDID[%s]   PID[%s] Similarity[%f]  ThresholdValue[%d] FDDesc[%s]\n",
        struAbsTime.dwYear, struAbsTime.dwMonth, struAbsTime.dwDay, struAbsTime.dwHour,
        ...
        struFaceSnapMatchAlarm.struRegion.fX, struFaceSnapMatchAlarm.struRegion.fY, struFaceSnapMatchAlarm.struRegion.fWidth, struFaceSnapMatchAlarm.struRegion.fHeight, struFaceSnapMatchAlarm.byPicTransType, tempFDID, temppID, struFaceSnapMatchAlarm.fSimilarity, struFaceSnapMatchAlarm.struBlackListInfo.wThresholdValue, tempFDDesc);
    AddLog(m_iCurDeviceIndex, ALARM_INFO_T, szInfoBuf);
...
void CClientDemoDlg::FaceSnapMatchAlarmInfo(WPARAM wParam, LPARAM lParam)
{
    ...
    sprintf(szInfoBuf, "Face Snap Match alarm FDID[%s] PID[%s] ThresholdValue[%d] FDDesc[%s] Abs[%4.4d%2.2d%2.2d%2.2d%2.2d%2.2d] FaceID[%d] BlackList ID[%d] StorageIP[%s]-StoragePort[%d] FaceScore[%d] Sex[%d] Glasses[%d] Age[%d] AgeDeviation[%d] MatchPicNum[%d] struRegion[%0.1f, %0.1f, %0.1f, %0.1f] PicTransType[%d]\n",
        tempFDID, temppID, struFaceSnapMatchAlarm.struBlackListInfo.wThresholdValue, tempFDDesc, struAbsTime.dwYear, struAbsTime.dwMonth, struAbsTime.dwDay, struAbsTime.dwHour,
        struAbsTime.dwMinute, struAbsTime.dwSecond, struFaceSnapMatchAlarm.struSnapInfo.dwSnapFacePicID, \
        struFaceSnapMatchAlarm.struBlackListInfo.struBlackListInfo.dwRegisterID, \
        struFaceSnapMatchAlarm.sStorageIP, struFaceSnapMatchAlarm.wStoragePort, \
        ...
        struFaceSnapMatchAlarm.struSnapInfo.byAge, struFaceSnapMatchAlarm.struSnapInfo.byAgeDeviation, struFaceSnapMatchAlarm.byMatchPicNum, \
...
void CClientDemoDlg::ProcITSPlateAlarm(WPARAM wParam, LPARAM lParam)
{
    ...
    sprintf(szInfoBuf, "ITS Plate Alarm PlateInfo PlateType[%d] Color[%d] Bright[%d] LicenseLen[%d]\
       EntireBelieve[%d] Region[%d] Country[%d] License[%s] PlateRect[x-%f y-%f w-%f h-%f] \
       VehicleInfo  VehicleType[%d] Color[%d] RadarState[%d] Speed[%d] VehicleLogoRecog[%d] VehicleSubLogoRecog[%d]", \
       ...
       struPlateResult.struPlateInfo.struPlateRect.fWidth, struPlateResult.struPlateInfo.struPlateRect.fHeight,
	...
	for (i = 0; i < 6; i++)
	{
		...
		{
			sprintf(szInfoBuf, "ITS picture[%d] info Type[%d] RedLightTime[%d] AbsTime[%s]  CloseUpType[%d] PlateRecgRect[x-%f y-%f w-%f h-%f]", i+1, struPlateResult.struPicInfo[i].byType,
...
void CClientDemoDlg::ProTagInfoUpload(WPARAM wParam, LPARAM lParam)
{
    ...
    sprintf(szInfoBuf, "%s: (Card No:[%s]  Rssi:[%d])\n", szLan, pStruTagInfoAlarm->byCardNo, pStruTagInfoAlarm->iRssi);
