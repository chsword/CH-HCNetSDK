...
void CDlgIpViewCfg::OnBtnGetSipCfg() 
{
	...
	{
		char szLan[128] = {0};
		g_pMainDlg->AddLog(m_dwDevIndex, OPERATION_FAIL_T, "NET_DVR_GET_SIP_CFG");
	...
	{
		g_pMainDlg->AddLog(m_dwDevIndex, OPERATION_SUCC_T, "NET_DVR_GET_SIP_CFG");	
	}
...
void CDlgIpViewCfg::OnBtnSetSipCfg() 
{
	...
	m_struSipCfg.dwSize = sizeof(NET_DVR_SIP_CFG);
    char szLan[256] = { 0 };
	...
	{
		char szLan[128] = {0};
		g_pMainDlg->AddLog(m_dwDevIndex, OPERATION_FAIL_T, "NET_DVR_SET_SIP_CFG");
		g_StringLanType(szLan, "参数设置失败", "Save failed");
		AfxMessageBox(szLan);
	}
	else
	{
		g_pMainDlg->AddLog(m_dwDevIndex, OPERATION_SUCC_T, "NET_DVR_SET_SIP_CFG");	
	}
