...
void CDlgAES::OnButtonUpdateKey() 
{
	...
	DWORD dwReturn = 0;
	if(!NET_DVR_GetDVRConfig(m_lUSerID, NET_DVR_GET_AES_KEY, 0xff, &strAes, sizeof(strAes), &dwReturn))
	{
...
{
	if(!NET_DVR_InquestStreamEncrypt(m_lUSerID, 0xff, m_comboState.GetCurSel()))
	{
...
void CDlgAES::OnButtonGetState() 
{
	...
	if(!NET_DVR_InquestGetEncryptState(m_lUSerID, 0xff, &bState))
	{
