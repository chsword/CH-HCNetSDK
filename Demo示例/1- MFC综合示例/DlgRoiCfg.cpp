...
void CDlgRoiCfg::OnBtnGetRoiinfo()
{
	...
	if (struRoiDetectCfg.byRoiDetectType == DETECT_FIX_ROI)
	{
		...
		m_csRoiName.Format("%s", struRoiDetectCfg.uRoiDetectInfo.strRoiFixRectCfg.szFixRoiName);	
	}
	else
	{
		m_bFixRoi = FALSE;
...
void CDlgRoiCfg::OnSelchangeComboRoiid() 
{
	...
	if (m_struRoiDetectCond[iAssVal].byRoiDetectType == DETECT_FIX_ROI)
	{
		...
		m_csRoiName.Format("%s", m_struRoiDetectCfg[iAssVal].uRoiDetectInfo.strRoiFixRectCfg.szFixRoiName);
