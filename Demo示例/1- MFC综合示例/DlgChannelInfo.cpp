...
// 	m_comboTransportType.SetCurSel((m_pChanInfo->dwProtocol>>31) & 0x01);
    ...
    m_comboProtocol.SetCurSel(g_struDeviceInfo[m_pChanInfo->iDeviceIndex].pStruChanInfo[m_pChanInfo->iChanIndex].dwLinkMode);
...
// 	}
	...
	g_struDeviceInfo[m_pChanInfo->iDeviceIndex].pStruChanInfo[m_pChanInfo->iChanIndex].dwStreamType = m_comboTransportType.GetCurSel();
    g_struDeviceInfo[m_pChanInfo->iDeviceIndex].pStruChanInfo[m_pChanInfo->iChanIndex].dwLinkMode = m_comboProtocol.GetCurSel();
    g_struDeviceInfo[m_pChanInfo->iDeviceIndex].pStruChanInfo[m_pChanInfo->iChanIndex].nPreviewProtocolType = m_pChanInfo->nPreviewProtocolType;
    g_struDeviceInfo[m_pChanInfo->iDeviceIndex].pStruChanInfo[m_pChanInfo->iChanIndex].dwPreviewMode = m_pChanInfo->dwPreviewMode;
    
    g_SaveDeviceConfig();

	if (m_comboPassbackRecord.GetCurSel() < 0)
    {
        g_struDeviceInfo[m_pChanInfo->iDeviceIndex].pStruChanInfo[m_pChanInfo->iChanIndex].bPassbackRecord = FALSE; 
    }
    else
    {
		g_struDeviceInfo[m_pChanInfo->iDeviceIndex].pStruChanInfo[m_pChanInfo->iChanIndex].bPassbackRecord = m_comboPassbackRecord.GetCurSel();
    }
    g_struDeviceInfo[m_pChanInfo->iDeviceIndex].pStruChanInfo[m_pChanInfo->iChanIndex].nPreviewProtocolType = m_cmPreviewProtocolType.GetCurSel();
	OnOK();
}
